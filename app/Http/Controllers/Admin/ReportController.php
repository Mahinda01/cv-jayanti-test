    ...
    public function index(Request $request)
    {
        ...
        $products = Product::with('category')
            ...
            ->map(function ($product) {
                return [
                    ...
                    'product_status' => $product->is_active ? 'Aktif' : 'Tidak Aktif',
                ];
            });
        ...
        $salesReport = $sales->map(function ($sale) {
            return [
                ...
                'remaining_amount_text' => $this->money($sale->remaining_amount),
        ...
        $receivableReport = $receivables->map(function ($sale) {
            ...
            return [
                ...
                'sale_date' => $this->dateText($sale->sale_date),
                'due_date' => $sale->due_date ? $this->dateText($sale->due_date) : '-',
                'total_amount' => $sale->total_amount,
                'total_amount_text' => $this->money($sale->total_amount),
                'paid_amount' => $sale->paid_amount,
                'paid_amount_text' => $this->money($sale->paid_amount),
        ...
        $receivablePaymentIncome = ReceivablePayment::whereDate('payment_date', '>=', $startDate)
            ->whereDate('payment_date', '<=', $endDate)
            ->sum('amount');

        $totalSalesAmount = $financeSales->sum('total_amount');
        $totalIncome = $cashSalesIncome + $creditInitialIncome + $receivablePaymentIncome;
        $totalReceivable = $financeSales->sum('remaining_amount');

        $saleItems = SaleItem::with('product')
            ->whereHas('sale', function ($query) use ($startDate, $endDate) {
                $query->where('transaction_status', 'Aktif')
                    ->whereDate('sale_date', '>=', $startDate)
                    ->whereDate('sale_date', '<=', $endDate);
            })
            ->get();
        ...
        return Inertia::render('Admin/Reports/Index', [
            ...
            'salesReport' => [
                'summary' => [
                    ...
                    'total_sales' => $this->money($sales->sum('total_amount')),
                    'total_paid' => $this->money($sales->sum('paid_amount')),
                    'total_remaining' => $this->money($sales->sum('remaining_amount')),
                ],
                'sales' => $salesReport,
            ],
            ...
            'stockReport' => [
                'summary' => [
                    ...
                    'low_stock_products' => $products->where('stock_status', 'Menipis')->count(),
                    'empty_stock_products' => $products->where('stock_status', 'Habis')->count(),
            ...
            'receivableReport' => [
                'summary' => [
                    ...
                    'total_receivable' => $this->money($receivables->sum('remaining_amount')),
                    'not_due_count' => $receivableReport->where('receivable_status', 'Belum Lunas')->count(),
                    'overdue_count' => $receivableReport->where('receivable_status', 'Jatuh Tempo')->count(),
                ],
            ...
            'financialReport' => [
                ...
                'rows' => [
                    [
                        ...
                        'description' => 'Total nilai transaksi aktif pada periode laporan.',
                    ],
                    [
                        'name' => 'Pemasukan Tunai / Transfer',
                        'amount' => $this->money($cashSalesIncome),
                        'description' => 'Pemasukan dari transaksi tunai dan transfer.',
                    ],
                    [
                        'name' => 'Uang Muka Transaksi Kredit',
                        'amount' => $this->money($creditInitialIncome),
                        'description' => 'Pembayaran awal dari transaksi kredit.',
                    ],
                    [
                        'name' => 'Pembayaran Piutang',
                        'amount' => $this->money($receivablePaymentIncome),
                        'description' => 'Pembayaran piutang yang diterima pada periode laporan.',
                    ],
                    [
                        'name' => 'Total Pemasukan Diterima',
                        'amount' => $this->money($totalIncome),
                        'description' => 'Total kas masuk yang diterima perusahaan.',
                    ],
                    [
                        'name' => 'Estimasi Modal Produk Terjual',
                        'amount' => $this->money($productCost),
                        'description' => 'Estimasi modal berdasarkan harga beli produk dan jumlah terjual.',
                    ],
                    [
                        'name' => 'Laba Kotor Penjualan',
                        'amount' => $this->money($grossProfit),
                        'description' => 'Total penjualan dikurangi estimasi modal produk terjual.',
                    ],
                    [
                        'name' => 'Laba / Rugi Kas Sementara',
                        'amount' => $this->money($cashProfit),
                        'description' => 'Total pemasukan diterima dikurangi estimasi modal produk terjual.',
                    ],
                    [
                        'name' => 'Sisa Piutang Berjalan',
                        'amount' => $this->money($totalReceivable),
                        'description' => 'Sisa pembayaran dari transaksi kredit yang belum lunas.',
                    ],
                ],
            ],
        ]);
